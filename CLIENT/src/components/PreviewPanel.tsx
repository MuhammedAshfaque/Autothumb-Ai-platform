  ...
  return (
    <div className='relative mx-auto w-full max-w-2xl'>
      <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}>
        ...
        {/* Image preview: When thumbnail available */}
        {!isLoading && thumbnail?.image_url && (
            <div className='group relative h-full w-full'>
                ...
                <div className='absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100'>
                    <button onClick={onDownload} className='md-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur hover:scale-105 active:scale-95' type='button'>
        ...
        {/* Means we dont given any prompt to generate thumbnail */}
        {!isLoading && !thumbnail?.image_url && (
            <div className='absolute inset-0 m-2 flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-white/20 bg-black/10'>
