...
const MyGeneration = () => {
  ...
  const fetchThumbnail = async () =>{
    try {
      ...
      toast.error(error?.response?.data?.message || "Failed to fetch thumbnails. Please try again." );
    }
    finally{
      setloading(false);
    }
              ...
              return (
                <div key={thumb._id} onClick={()=> navigate(`/generate/${thumb._id}`)} className='mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid'>
                  ...
                  <div onClick={(e)=> e.stopPropagation()} className='absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5'>
                    <Trash2Icon onClick={() => handleDelete(thumb._id)} className="size-6 bg-black/50 p-1 rounded hover:bg-blue-600 transition-all"/>
                    ...
                    <Link target='_blank' to={`/preview?.thumbnail_url=${thumb.image_url} && title=${thumb.title}`}><ArrowUpRight className='size-6 bg-black/50 p-1 rounded hover:bg-blue-600 transition-all'/></Link>
                  </div>
                </div>
              )
            })}
